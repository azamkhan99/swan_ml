...
def select_features(target, run_id_model_mapping):
    ...
    artifact_path = f"{target.split(' ')[0]}_ffnn_input_features.json"
...
def main():
    ...
    run_id_model_mapping = {
        "Sediments DlyLd(kg*1000)": "165e2149ed2041f288253c5e102a3432",
        "Nitrate DlyLd(kg)": "a252404ca8d4463dbe16d7532c6a1a4b",
        "Phosphate DlyLd(kg)": "555c64fa7c19442dbe69880ae91e48c8",
    }
    ...
    for climate_model in climate_models:
        for emission_scenario in emission_scenarios:
            ...
            for target in swat_quals_dfs.keys():
                ...
                mlflow_model_uri = f"runs:/{run_id_model_mapping[target]}/{target.split(' ')[0]}_ffnn_model"
                ...
                loaded_model.eval()
                with torch.no_grad():
                    y_pred = loaded_model(X_tensor).reshape(-1).cpu().numpy()
                # y_pred = sv.data
                ...
                output_dir = (
                    f"ml_prediction_outputs_ffnn/{climate_model}/{emission_scenario}"
                )
                # logging.info(
