...
run_id_model_mapping = {
    "Sediments DlyLd(kg*1000)": "e6670958198e40be92b7d99952c18a0c",
    "Nitrate DlyLd(kg)": "3dd78ea0557c4079936b60532d190a51",
    "Phosphate DlyLd(kg)": "7d6f1e9e13a34d6aa24642579da78081",
}
...
def run(log_transform=False):
    ...
    for target in targets:
        if log_transform:
            experiment_run_name = f"{target.split()[0]}_ffnn_log_infer"
        else:
            experiment_run_name = f"{target.split()[0]}_ffnn_infer"
        with mlflow.start_run(run_name=experiment_run_name):
            ...
            dataset_path = f"mlflow_datasets/ffnn_{target}_X_train_scaled.csv"
            ...
            model = learnKappa_layers2(
                X_train_scaled.shape[1],
                int(params["hid_nodes"]),
                1,
            ).to(device)
            model.load_state_dict(
                torch.load(
                    f"models/ffnn_{target.replace(' ', '_')}.pt", map_location=device
                )
            )

            mlflow.pytorch.log_model(
                model,
                artifact_path=f"{target.split()[0]}_ffnn_model",
                registered_model_name=f"{target.split()[0]}_ffnn_model",
            )
