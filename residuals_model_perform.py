...
run_id_model_mapping = {
    "Sediments DlyLd(kg*1000)": "69dd3d1d3d8b4a168bdf266ea6ed03a7",
    "Nitrate DlyLd(kg)": "9cbe9c63f1174037bd59053985a0ccd7",
    "Phosphate DlyLd(kg)": "49dcd17889994fc291196e458c6ae5b8",
}
...
def run(log_transform=False):
    ...
    for target in targets:
        ...
        if log_transform:
            experiment_run_name = f"{target.split()[0]}_residual_log_infer"
        else:
            experiment_run_name = f"{target.split()[0]}_residual_infer"
        with mlflow.start_run(run_name=experiment_run_name):
            ...
            dataset_path = f"mlflow_datasets/residual_{target}_X_train_scaled.csv"
            ...
            if log_transform:
                ...
                # log transformer to mlflow
                mlflow.sklearn.log_model(
                    residual_transformer,
                    f"{target}_residual_transformer",
                )
                # log offset to mlflow
                mlflow.log_param("logOffset", offset)
            else:
            ...
            model = ImprovedNN(
                input_size,
                json.loads(params["hidden_sizes"]),
                output_size,
                dropout_rate=0.3,
            ).to(device)
            model.load_state_dict(
                torch.load(
                    f"models/residual_{target.replace(' ', '_')}.pt",
                    map_location=device,
                )
            )
            ...
            mlflow.pytorch.log_model(
                model,
                artifact_path=f"{target.split()[0]}_residual_model",
                registered_model_name=f"{target.split()[0]}_residual_model",
            )
            ...
            mape = mean_absolute_percentage_error(y_actual[target], y_pred_corrected)
            mlflow.log_metrics(
